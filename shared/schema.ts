...
// Helper documents table (헬퍼 서류 관리 - 개별 서류별 상태 추적)
export const helperDocuments = pgTable("helper_documents", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  ...
  // 한 유저당 각 서류 타입별로 하나의 레코드만
  uniqueUserDocument: unique().on(table.userId, table.documentType),
}));
...
export const termsVersions = pgTable("terms_versions", {
  id: serial("id").primaryKey(),
  termsType: text("terms_type").notNull(), // "helper_signup" | "requester_signup" | "helper_contract" | "requester_order" | "privacy_policy" | "location_policy"
  version: text("version").notNull(), // "1.0.0", "1.1.0" 등
  title: text("title").notNull(), // 약관 제목
  content: text("content").notNull(), // 약관 전문
  summary: text("summary"), // 변경 요약
  isActive: boolean("is_active").default(true), // 현재 적용 중인 버전
  effectiveDate: timestamp("effective_date").notNull(), // 시행일
  createdAt: timestamp("created_at").defaultNow(),
});

// ============ 가입 시 동의 기록 테이블 ============
export const signupConsents = pgTable("signup_consents", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  role: text("role").notNull(), // "helper" | "requester"
  // 필수 동의 항목
  termsAgreed: boolean("terms_agreed").notNull(), // 서비스 이용약관
  privacyAgreed: boolean("privacy_agreed").notNull(), // 개인정보 수집·이용
  locationAgreed: boolean("location_agreed").notNull(), // 위치정보 수집·이용
  paymentAgreed: boolean("payment_agreed").notNull(), // 결제·정산 정책
  liabilityAgreed: boolean("liability_agreed").notNull(), // 책임 제한 동의
  electronicAgreed: boolean("electronic_agreed").notNull(), // 전자계약 및 고지
  // 헬퍼 전용 필수 동의
  industrialAccidentInsuranceAgreed: boolean("industrial_accident_insurance_agreed"), // 산재보험 안내 확인 및 보험료 원천공제 동의
  cargoInsuranceAgreed: boolean("cargo_insurance_agreed"), // 적재물배상책임보험 안내 확인
  independentContractorAgreed: boolean("independent_contractor_agreed"), // 독립사업자(비고용) 관계 확인
  // 선택 동의
  marketingAgreed: boolean("marketing_agreed").default(false), // 마케팅 수신 동의
  // 약관 버전 기록 (어떤 버전에 동의했는지)
  termsVersionId: integer("terms_version_id").references(() => termsVersions.id, { onDelete: "set null" }),
  privacyVersionId: integer("privacy_version_id").references(() => termsVersions.id, { onDelete: "set null" }),
  // 법적 증거 기록
  ipAddress: text("ip_address"),
  userAgent: text("user_agent"),
  consentLog: text("consent_log"), // JSON: 각 항목별 동의 시각 기록
  agreedAt: timestamp("agreed_at").defaultNow(),
  createdAt: timestamp("created_at").defaultNow(),
});

// ============ 약관 변경 재동의 기록 ============
export const termsReConsents = pgTable("terms_re_consents", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  termsVersionId: integer("terms_version_id")
    .notNull()
    .references(() => termsVersions.id, { onDelete: "cascade" }),
