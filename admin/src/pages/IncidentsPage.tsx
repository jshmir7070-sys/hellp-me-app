...
export default function IncidentsPage() {
  ...
  const columns: ColumnDef<Incident>[] = useMemo(() => [
    ...
    },
    {
      ...
      header: '유형',
      cell: ({ row }) => <Badge variant="outline">{TYPE_LABELS[row.original.incidentType] || row.original.incidentType}</Badge>,
    },
    {
      accessorKey: 'description',
      header: '내용',
      cell: ({ row }) => <span className="text-sm truncate block max-w-xs">{row.original.description}</span>,
    },
    {
      accessorKey: 'requestedAmount',
      header: '요청 금액',
      cell: ({ row }) => row.original.requestedAmount ? (
        <span className="font-medium">{row.original.requestedAmount.toLocaleString()}원</span>
      ) : (
    ...
    },
    {
      ...
      header: '차감액',
      cell: ({ row }) => row.original.deductionAmount ? (
        <span className="font-medium text-red-600">-{row.original.deductionAmount.toLocaleString()}원</span>
      ) : (
        <span className="text-muted-foreground">-</span>
      ),
    },
    {
      accessorKey: 'evidenceUrls',
      header: '증빙',
      cell: ({ row }) => {
        const urls = row.original.evidenceUrls || [];
    ...
    },
    {
      ...
      header: '답변',
      cell: ({ row }) => row.original.adminReply ? (
    ...
    {
      accessorKey: 'helperStatus',
      header: '헬퍼응답',
      cell: ({ row }) => {
        if (row.original.adminForceProcessed) {
          return <Badge className="bg-orange-100 text-orange-800">강제처리</Badge>;
        }
        if (row.original.helperStatus === 'confirmed') {
          return <Badge className="bg-green-100 text-green-800">확인완료</Badge>;
        }
        if (row.original.helperStatus === 'item_found') {
          return <Badge className="bg-blue-100 text-blue-800">물건찾음</Badge>;
        }
        if (row.original.helperStatus === 'request_handling') {
          return <Badge className="bg-purple-100 text-purple-800">처리요망</Badge>;
        }
        // 헬퍼 응답 기한 체크
        if (row.original.helperResponseDeadline) {
          if (isDeadlinePassed(row.original.helperResponseDeadline)) {
            return <Badge className="bg-red-100 text-red-800">기한초과</Badge>;
          }
          return <span className="text-xs text-amber-600">{formatDeadline(row.original.helperResponseDeadline)}</span>;
        }
    ...
    },
    {
      ...
      header: '액션',
      cell: ({ row }) => (
        <Button size="sm" variant="ghost" onClick={() => openDetail(row.original)}>
  ...
  const stats = useMemo(() => {
    ...
    const pending = incidents.filter((i: Incident) => i.status === 'pending' || i.status === 'submitted').length;
    const reviewing = incidents.filter((i: Incident) => i.status === 'reviewing').length;
    const resolved = incidents.filter((i: Incident) => i.status === 'resolved' || i.status === 'applied').length;
    const totalRequested = incidents.reduce((sum: number, i: Incident) => sum + (i.requestedAmount || 0), 0);
    const totalDeducted = incidents.reduce((sum: number, i: Incident) => sum + (i.deductionAmount || 0), 0);
    return { all, pending, reviewing, resolved, totalRequested, totalDeducted };
  }, [incidents]);
  ...
  return (
    <div className="space-y-6">
      ...
      <StatsGrid columns={6}>
        ...
        <StatsCard
          title="총 요청 금액"
          value={`${stats.totalRequested.toLocaleString()}원`}
          icon={<AlertTriangle className="h-4 w-4" />}
        />
        <StatsCard
          title="총 차감 금액"
          value={`${stats.totalDeducted.toLocaleString()}원`}
          icon={<AlertTriangle className="h-4 w-4" />}
        />
      </StatsGrid>
        ...
        <CardContent className="p-0">
          {incidents.length === 0 && !isLoading ? (
            <EmptyState
              ...
              title="등록된 사고가 없습니다"
              description="새로운 사고를 등록하여 관리를 시작하세요"
            />
          ) : (
            <DataTable
              columns={columns}
