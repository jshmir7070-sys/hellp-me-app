...
export default function DailySettlementPage() {
  ...
  const { data: settlementsResponse, isLoading } = useQuery({
    queryKey: ['/api/admin/settlements/daily', dateRange.from, dateRange.to, searchTerm, currentPage, itemsPerPage],
    queryFn: async () => {
      ...
      const res = await adminFetch(`/api/admin/settlements/daily?${params.toString()}`);
  ...
  const pagination = settlementsResponse?.pagination;
