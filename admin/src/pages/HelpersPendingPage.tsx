...
import { Check, X, Eye, Users, RefreshCw, Download, FileText, CreditCard, Car, Building2, Clock, UserPlus, CheckCircle } from 'lucide-react';
...
export default function HelpersPendingPage() {
  ...
  const stats = useMemo(() => {
    ...
    const todayCount = helpers.filter((h: PendingHelper) => new Date(h.createdAt) >= today).length;
    const docsComplete = helpers.filter((h: PendingHelper) => h.licenseVerified && h.bankAccountVerified).length;
    const completionRate = helpers.length > 0 ? Math.round((docsComplete / helpers.length) * 100) : 0;
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-1">
          <Badge variant={row.original.licenseVerified ? 'default' : 'outline'} className="text-xs">
            면허 {row.original.licenseVerified ? '완료' : '미완료'}
          </Badge>
          <Badge variant={row.original.bankAccountVerified ? 'default' : 'outline'} className="text-xs">
            계좌 {row.original.bankAccountVerified ? '완료' : '미완료'}
          </Badge>
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="신규 헬퍼 승인"
        description="신규 가입한 헬퍼의 서류 확인 및 승인 관리 • 실시간 알림"
        actions={
          <>
      ...
      {helpers.length === 0 ? (
        <EmptyState
          ...
          title="승인 대기 중인 헬퍼가 없습니다"
          description="모든 헬퍼가 승인되었거나 대기 중인 신청이 없습니다."
        />
      ) : (
        <DataTable
          columns={columns}
