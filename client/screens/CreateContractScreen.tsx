...
export default function CreateContractScreen({ route, navigation }: Props) {
  ...
  // 진행률 계산 (complete 제외)
  const getStepNumber = (s: PaymentStep): number => {
    const stepMap = { 'contract': 1, 'signature': 2, 'verification': 3, 'payment': 4, 'complete': 4 };
    return stepMap[s];
  };
  ...
  const renderStepIndicator = () => (
    <View style={styles.stepIndicator}>
      {/* 현재 단계 표시 */}
      <View style={[styles.currentStepBanner, { backgroundColor: isDark ? '#1a365d' : '#EBF8FF' }]}>
        <View style={styles.currentStepContent}>
          <View style={[styles.currentStepCircle, { backgroundColor: BrandColors.requester }]}>
            <ThemedText style={styles.currentStepText}>{currentStep}</ThemedText>
          </View>
          <View style={styles.currentStepInfo}>
            <ThemedText style={[styles.currentStepLabel, { color: BrandColors.requester }]}>
              {step === 'contract' && '계약서 확인'}
              {step === 'signature' && '전자서명'}
              {step === 'verification' && '본인인증'}
              {step === 'payment' && '계약 확정'}
              {step === 'complete' && '완료'}
            </ThemedText>
            <ThemedText style={[styles.currentStepSubLabel, { color: Colors.light.tabIconDefault }]}>
      ...
      <View style={styles.stepRow}>
        <View style={[styles.stepCircle, isStepActive('contract') && styles.activeStep, isStepCompleted('contract') && styles.completedStep]}>
          {isStepCompleted('contract') ? <Icon name="checkmark-outline" size={14} color="#fff" /> : <ThemedText style={styles.stepNumber}>1</ThemedText>}
        </View>
        ...
        <View style={[styles.stepCircle, isStepActive('signature') && styles.activeStep, isStepCompleted('signature') && styles.completedStep]}>
          {isStepCompleted('signature') ? <Icon name="checkmark-outline" size={14} color="#fff" /> : <ThemedText style={styles.stepNumber}>2</ThemedText>}
        </View>
        ...
        <View style={[styles.stepCircle, isStepActive('verification') && styles.activeStep, isStepCompleted('verification') && styles.completedStep]}>
          {isStepCompleted('verification') ? <Icon name="checkmark-outline" size={14} color="#fff" /> : <ThemedText style={styles.stepNumber}>3</ThemedText>}
        </View>
        ...
        <View style={[styles.stepCircle, isStepActive('payment') && styles.activeStep, isStepCompleted('payment') && styles.completedStep]}>
          {isStepCompleted('payment') ? <Icon name="checkmark-outline" size={14} color="#fff" /> : <ThemedText style={styles.stepNumber}>4</ThemedText>}
        </View>
      </View>
      
      <View style={styles.stepLabels}>
        <ThemedText style={[styles.stepLabelSmall, { color: isStepActive('contract') ? BrandColors.requester : theme.tabIconDefault, fontWeight: isStepActive('contract') ? '600' : '400' }]}>계약확인</ThemedText>
        <ThemedText style={[styles.stepLabelSmall, { color: isStepActive('signature') ? BrandColors.requester : theme.tabIconDefault, fontWeight: isStepActive('signature') ? '600' : '400' }]}>서명</ThemedText>
        <ThemedText style={[styles.stepLabelSmall, { color: isStepActive('verification') ? BrandColors.requester : theme.tabIconDefault, fontWeight: isStepActive('verification') ? '600' : '400' }]}>본인인증</ThemedText>
        <ThemedText style={[styles.stepLabelSmall, { color: isStepActive('payment') ? BrandColors.requester : theme.tabIconDefault, fontWeight: isStepActive('payment') ? '600' : '400' }]}>확정</ThemedText>
      </View>
    </View>
  );
  ...
  const renderContractStep = () => (
    <>
      ...
      <Card style={[styles.card, { backgroundColor: isDark ? '#2d3748' : '#F7FAFC' }]}>
