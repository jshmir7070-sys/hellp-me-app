...
type RequesterClosingScreenProps = NativeStackScreenProps<HomeStackParamList, 'RequesterClosing'>;
...
export default function RequesterClosingScreen({ navigation }: RequesterClosingScreenProps) {
  ...
  const handleCardPress = useCallback((item: OrderCardDTO) => {
    ...
    if (!isNaN(orderId)) {
      navigation.navigate('ClosingDetail' as any, { orderId });
    }
  }, [navigation]);
  ...
  const handleCardAction = useCallback((action: string, item: OrderCardDTO) => {
    ...
    switch (action) {
      ...
      case 'confirm_closing':
        ...
        break;
      case 'pay_balance':
        navigation.navigate('Payment' as any, { orderId, type: 'balance' });
        break;
    }
