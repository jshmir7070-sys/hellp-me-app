...
const FAQ_SECTIONS: FAQSection[] = [
  ...
  },
  {
    ...
    items: [
      {
        question: '오더를 삭제하거나 숨기고 싶어요',
        answer: '오더 삭제/숨기기 안내 (요청자):\n\n[오더 삭제]\n• 매칭 전 상태에서만 가능\n• 오더 상세 페이지 하단 > "삭제" 버튼\n• 삭제 시 계약금 100% 환불\n\n[오더 숨기기]\n• 완료된 오더를 목록에서 숨기기\n• 오더 상세 페이지 하단 > "숨기기" 버튼\n• 마감 완료, 정산 완료 등 종료 상태 오더 대상\n• 숨긴 오더는 목록에서 보이지 않음\n\n[주의]\n• 헬퍼 선택 후에는 삭제 불가\n• 진행 중 오더 취소는 고객센터 문의',
      },
      {
      ...
      {
        question: '오더 조건을 수정하고 싶어요 (요청자)',
        answer: '오더 수정 방법:\n\n[수정 가능 시점]\n• 매칭 전 상태에서만 가능\n• (open, registered, matching, awaiting_deposit)\n\n[수정 방법]\n오더 상세 페이지 하단 > "수정" 버튼\n\n[수정 가능 항목]\n• 단가 (박스당 금액)\n• 작업 날짜\n\n[수정 불가 항목]\n• 택배사, 지역, 물량 등 기타 조건\n• 이 항목들은 오더 삭제 후 새로 등록 필요\n\n[헬퍼 선택 후]\n• 수정 불가\n• 협의 필요시 고객센터 문의',
      },
      {
        question: '매칭이 취소되었어요',
        answer: '매칭 취소 사유:\n\n[자동 취소]\n• 헬퍼가 QR 체크인을 하지 않음\n• 양측 합의로 취소\n• 관리자 판단으로 취소\n\n[취소 후 처리]\n• 요청자: 오더 재공개 또는 환불\n• 헬퍼: 다른 오더 지원 가능\n\n[재매칭]\n• 오더 상세 페이지에서 "재공개" 버튼\n• 새로운 헬퍼 지원 대기',
      ...
      {
        question: '오더가 보이지 않아요 (헬퍼)',
        answer: '오더가 안 보이는 경우:\n\n[확인사항]\n1. 서류 승인이 완료되었는지\n2. 앱이 최신 버전인지\n3. 오더 탭의 필터가 설정되어 있는지\n4. 인터넷 연결 상태\n\n[해결방법]\n• 서류 미승인: 나의정보 > 서류 제출에서 승인 상태 확인\n  (사업자등록증, 운전면허증, 화물운송종사자격증, 차량등록증)\n• 필터 해제: 오더 탭 필터 초기화\n• 앱 새로고침: 화면 아래로 당기기\n• 재로그인: 로그아웃 후 다시 로그인',
      },
      {
        question: '홈 화면에서 헬퍼를 어떻게 선택하나요? (요청자)',
        answer: '헬퍼 선택 방법 (요청자):\n\n[홈 화면 지원자 카드]\n1. 홈 화면에서 지원한 헬퍼 카드 확인\n2. 카드 터치 시 헬퍼 프로필 팝업 표시\n3. 프로필에서 경력, 리뷰, 평점 확인\n4. "선택" 버튼으로 해당 헬퍼 확정\n\n[지원자 목록 확인]\n• 오더 상세 페이지 > 지원자 목록\n• 여러 지원자 비교 후 선택\n\n[선택 후]\n• 헬퍼에게 알림 발송\n• 계약서 서명 진행\n• 출근일에 QR 체크인 필요',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        answer: '마감 제출 방법 (헬퍼):\n\n[제출 위치]\n오더 상세 페이지 하단 > "마감 제출" 버튼\n\n[오더 상세 접근]\n• 홈 캘린더에서 해당 날짜 오더 카드 터치\n• 또는 오더 탭에서 해당 오더 선택\n\n[입력 항목]\n1. 배송 완료 건수\n2. 반품 건수 (있는 경우)\n3. 배송 이력 이미지 (필수)\n   - 쿠팡앱/배송앱 캡처\n4. 기타 이미지 (선택)\n   - 특이사항 증빙\n\n[제출 완료]\n• "마감 제출" 버튼 터치\n• 요청자에게 알림 발송\n• 요청자 승인 대기',
      },
      {
        question: '마감 내용을 수정하고 싶어요',
        answer: '마감 수정 방법:\n\n[제출 전]\n• 입력 내용 자유롭게 수정 가능\n• 이미지 추가/삭제 가능\n\n[제출 후]\n• 직접 수정 불가\n• 요청자에게 반려 요청 필요\n• 반려 후 수정하여 재제출\n\n[반려 요청 방법]\n• 요청자에게 연락\n• 반려 사유 설명\n• 요청자가 오더 상세에서 "반려" 버튼 터치\n• 반려 후 마감 재제출',
      },
      {
        question: '출근 체크가 안 돼요',
        answer: 'QR 체크인 문제 해결:\n\n[출근 체크 접근 방법 (3가지)]\n① 홈 화면 캘린더에서 배정된 오더 카드 터치 → 출근 체크 화면\n② 나의정보 > 출근 체크 메뉴\n③ 오더 상세 페이지 하단 > QR 체크인 버튼\n\n[QR 스캔 안 될 때]\n• 카메라 권한 확인 (설정 > 헬프미 > 카메라)\n• QR 코드가 선명한지 확인\n• 조명 충분한지 확인\n\n[대안: 12자리 코드 직접 입력]\n1. 출근 체크 화면에서 코드 입력란에\n2. 요청자의 12자리 고유 코드 입력\n3. 확인 버튼 터치\n\n[요청자 코드 확인 방법]\n• 요청자: 나의정보 > 출근 체크에서 QR/코드 확인\n• 코드 복사 또는 공유 기능 제공\n\n[계속 안 되면]\n• 앱 재시작\n• 고객센터 문의',
      },
      {
        question: '마감 확인이 안 돼요 (요청자)',
        answer: '마감 확인 문제 해결:\n\n[마감이 안 보이는 경우]\n• 헬퍼가 아직 마감 제출 안 함\n• 앱 새로고침 (화면 아래로 당기기)\n\n[확인 방법]\n오더 상세 페이지에서 마감 내역 확인\n• 제출된 내역 확인\n• 승인 또는 반려 선택\n\n[승인/반려]\n• 승인: 잔금 결제 진행\n• 반려: 사유 입력 후 헬퍼에게 알림',
      },
      {
        question: '마감이 반려되었어요',
        answer: '마감 반려 시 대응 (헬퍼):\n\n[반려 확인]\n• 푸시 알림으로 반려 알림\n• 오더 상세 페이지에서 반려 사유 확인\n• 반려된 오더 카드는 빨간 테두리로 표시\n\n[흔한 반려 사유]\n• 배송 건수 불일치\n• 이미지가 불분명함\n• 반품 수량 오류\n• 정보 누락\n\n[재제출]\n1. 반려된 오더 상세 페이지 이동\n2. 반려 사유 확인\n3. 내용 수정\n4. "마감 제출" 다시 터치\n\n[문제 해결]\n• 사유가 이해 안 되면 요청자 연락\n• 계속 반려되면 고객센터 문의',
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: '서류 승인 안내:\n\n[필요 서류 (헬퍼)]\n• 사업자등록증\n• 운전면허증\n• 화물운송종사자격증\n• 차량등록증\n\n[서류 제출 위치]\n나의정보 > 서류 제출\n\n[승인 소요 시간]\n• 제출 후 1-2영업일\n• 주말/공휴일 제외\n\n[승인 결과]\n• 승인: 푸시 알림 + 홈 화면 배너 상태 변경\n• 반려: 반려 사유와 함께 알림\n\n[반려된 경우]\n• 반려 사유 확인\n• 서류 재제출\n• 다시 1-2영업일 소요',
      },
      {
        question: '홈 화면 캘린더는 뭔가요?',
        answer: '홈 화면 캘린더 안내:\n\n[헬퍼 홈 화면]\n• 상단 캘린더에서 배정된 오더 일정 확인\n• 날짜에 점 표시 = 해당일 오더 있음\n• 날짜 터치 시 해당일 오더 카드 표시\n• 배정/스케줄 상태 오더 카드 터치 시 바로 출근 체크 화면으로 이동\n\n[요청자 홈 화면]\n• 캘린더에서 등록한 오더 일정 확인\n• 날짜 터치 시 해당일 오더 카드 표시\n• 지원한 헬퍼 카드로 빠른 선택 가능\n\n[팁]\n• 캘린더로 일정 관리가 편리\n• 오더 탭에서는 전체 오더 목록 확인',
      },
      {
      ...
      {
        question: '앱의 주요 탭은 뭐가 있나요?',
        answer: '앱 구성 안내:\n\n[하단 탭 메뉴]\n• 홈: 캘린더 기반 오더 일정 관리\n• 오더 탭: 전체 오더 목록 (모집중/진행중 등)\n• 오더 등록 (요청자): 새 오더 등록\n• 나의정보: 프로필, 서류, 설정, 이력\n\n[홈 화면]\n• 캘린더로 일정 한눈에 확인\n• 오더 카드로 빠른 접근\n\n[오더 탭]\n• 필터별 오더 검색\n• 오더 카드 터치 시 상세 페이지 이동\n\n[나의정보]\n• 프로필 수정, 서류 제출\n• 출근 체크, 이력 확인\n• 앱 설정, 고객센터',
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        question: '이의제기/사고 신고는 어떻게 하나요?',
        answer: '이의제기 및 사고 신고:\n\n[이의제기 (정산 관련)]\n• 나의정보 > 이의제기 접수\n• 해당 오더와 금액 불일치 내용 작성\n• 증빙 자료 첨부\n• 관리자 검토 후 조정\n\n[사고 신고]\n• 요청자: 나의정보 > 사고 내역 > 사고 신고\n• 헬퍼: 나의정보 > 사고 내역에서 확인\n• 사고 내용 상세 작성 + 증빙 첨부\n\n[처리 절차]\n• 관리자 검토 (1-3영업일)\n• 상대방 확인\n• 조치 및 결과 통보',
      },
      {
