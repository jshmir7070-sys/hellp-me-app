 ...
 *
 * ⚠️ 중요: 모든 정산 금액은 이 파일의 함수를 통해서만 계산됩니다.
 * 헬퍼 앱, 요청자 앱, 관리자 패널 모두 동일한 결과를 반환합니다.
 *
 * 📋 반올림 규칙 (ROUNDING_RULES):
 * - VAT (부가세): Math.round() - 반올림
 * - 계약금 (depositAmount): Math.floor() - 내림 (고객 유리)
 * - 잔금 (balanceAmount): 계산값 (총액 - 계약금)
 * - 총액 (totalAmount): Math.round() - 반올림
 *
 * 🚫 주의: 다른 파일의 정산 계산 로직 사용 금지
 * - server/utils/settlement-calculator.ts - Policy 기반 (특수 케이스만)
 * - server/utils/settlement-calc.ts - Deprecated (사용 금지)
 */
